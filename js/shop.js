...
function getAllCategories(){
    $.getJSON(app+"/data/categories.json", function(data) {
        ...
        let categoryList_2=document.getElementById('put-cats-here');
        let categoryList_3=document.getElementById('put-shuffle-categories-here');
        $.each(data, function(key, category) {
            let item = new Category(category.id, category.name);
            categoryList.appendChild(createCategoryDom1(item));
            categoryList_2.appendChild(createCategoryDom2(item));
            categoryList_3.appendChild(createCategoryDom3(item));
        });
...
getAllCategories();
function getAllTrendsProducts(){
    var father = document.getElementById('put-your-trends-products-here');
    $.getJSON(app+"/data/products.json", function(data) {
        ...
        $.each(data, function(key, product) {
            ...
            if(item.trend){
                father.innerHTML+=createTrendProductDom(item);
            }
        });
...
function createCategoryDom1(informatoin){
...
function createCategoryDom3(informatoin){
    let link=document.createElement('li');
    link.className='btn btn-sm my-button ml-2';
    link.innerHTML=informatoin.name.toUpperCase();
    link.setAttribute('data-filter','.'+informatoin.name);
    return link;
}
function createCategoryDom2(informatoin){
    let item=document.createElement('li');
    item.className='list-group-item hvr-underline-from-right hover-color';
    item.innerHTML=informatoin.name;
    return item;
}
...
function createTrendProductDom(information){
    let category=getCategoryById(information.category).name;
let container=` <div class="item ${category} col-lg-3 col-md-4 col-6 col-sm">`;
container+=`<a href="${app+'/images/'+information.mainPhoto}" class="fancylight popup-btn" data-fancybox-group="light">`
container+=`<img class="img-fluid" src="${app+'/images/'+information.mainPhoto}"></a>
                       </div>`;
return container;
}
...
//     productName.className = 'title';
//     let productLink=document.createElement('a');
//     productLink.href='http://localhost/ITI/projects/details.html?id='+information.id;
//     productLink.innerHTML=information.name;
//     productName.appendChild(productLink);
//
//     let productPrice = document.createElement('div');
